...
/// Representa una cara triangular de la malla de la esfera.
#[derive(Clone, Copy)]
pub struct SphereTriangleIndices {
    pub a: usize,
    pub b: usize,
    pub c: usize,
}

/// Datos de la malla esférica.
pub struct SphereMesh {
    pub vertices: Vec<Vec3>,
    pub indices: Vec<SphereTriangleIndices>,
}

pub struct Sphere3D;

impl Sphere3D {
    /// Genera la malla procedural de una esfera centrada en el origen `(0, 0, 0)`.
    ///
    /// - `radius`: Radio de la esfera.
    /// - `lat_rings`: Número de divisiones latitudinales (de polo norte a polo sur).
    /// - `lon_segments`: Número de divisiones longitudinales (alrededor del eje Y).
    pub fn create_mesh(radius: f32, lat_rings: usize, lon_segments: usize) -> SphereMesh {
        let mut vertices = Vec::new();
        let mut indices = Vec::new();

        // 1. Generar vértices mediante coordenadas esféricas
        for i in 0..=lat_rings {
            // phi va de PI/2 (Polo Norte) a -PI/2 (Polo Sur)
            let phi = (PI * 0.5) - (PI * (i as f32 / lat_rings as f32));
        ...
        // 2. Generar índices de triángulos conectando los anillos
        ...
        for i in 0..lat_rings {
            for j in 0..lon_segments {
                ...
                indices.push(SphereTriangleIndices {
                    a: curr,
                    b: next,
                    c: curr + 1,
                });
    ...
    /// Renderiza la esfera proyectando sus triángulos en pantalla con sombreado plano.
    pub fn draw(
        ...
        let mut sorted_triangles: Vec<(&SphereTriangleIndices, f32)> = mesh
            .indices
            .iter()
            .filter_map(|tri| {
        ...
        // Renderizado con sombreado plano (Flat Shading basado en normales por cara)
        for (tri, _) in sorted_triangles {
            ...
            // Cálculo del vector normal de la cara triangular
            ...
            let shade = if len > 0.0 {
                // Dirección de luz direccional [0.4, 0.7, 0.3]
